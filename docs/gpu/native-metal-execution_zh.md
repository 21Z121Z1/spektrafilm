...
拟合反解曲线继续由 CPU 准备；相关模型域问题按下文明确报告。
...
首轮真机验证发现准备阶段的两处限制过严：Provia 的默认 DIR 反解轴并不单调，
Portra 拟合颗粒密度轴的肩部也有回折，CPU 本来就会处理这两种数据。新后端发出
`CanonicalModelWarning`，不改 profile 数值，保留 CPU 的 DIR 准备和颗粒二分搜索
行为。不排序、不强制单调，也不声称这些轴形成了有效的数学反函数。通用曲线
算子仍拒绝非单调用户输入；新增真机测试直接对比真实 CPU 插值器的非单调颗粒
行为。修复这些来源模型的曲线是另一个任务，不能混进后端移植。
