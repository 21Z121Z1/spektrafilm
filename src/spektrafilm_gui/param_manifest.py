...
GRAIN_MANIFEST = GroupManifest(
    ...
    fields=(
        ...
        ParamSpec(
            f"{_G}.agx_particle_scale",
            tooltip="Scale of particle area for the RGB layers, multiplies agx_particle_area_um2",
        ),
        ParamSpec(
            f"{_G}.agx_particle_scale_layers",
            min=0,
            step=0.25,
            tooltip="Scale of particle area for the sublayers in every color layer, multiplies agx_particle_area_um2",
        ),
...
TUNE_PANEL_FIELDS = (
    ...
    ParamSpec(
        ...
        name="film_base_active",
        label="Film base tuning",
        tier="basic",
        tooltip="Enable spectral tuning of the film base/fog and orange-mask density.",
    ),
    ParamSpec(
        "film_render.base.scale",
        name="film_base_scale",
        label="Film base scale",
        tier="basic",
        min=0.0,
        step=0.05,
        tooltip="Global multiplicative scale for the film-base spectral density.",
    ),
    ParamSpec(
        "film_render.base.cyan",
        name="film_base_cyan",
        label="Film base cyan",
        tier="basic",
        min=0.0,
        step=0.05,
        tooltip="Multiplicative film-base density shaping around the cyan/long-wavelength Status-M region; 1.0 is neutral.",
    ),
    ParamSpec(
        "film_render.base.magenta",
        name="film_base_magenta",
        label="Film base magenta",
        tier="basic",
        min=0.0,
        step=0.05,
        tooltip="Multiplicative film-base density shaping around the magenta/mid-spectrum Status-M region; 1.0 is neutral.",
    ),
    ParamSpec(
        "film_render.base.yellow",
        name="film_base_yellow",
        label="Film base yellow",
        tier="basic",
        min=0.0,
        step=0.05,
        tooltip="Multiplicative film-base density shaping around the yellow/short-wavelength Status-M region; 1.0 is neutral.",
    ),
    ParamSpec(
        "film_render.base.tilt",
        name="film_base_tilt",
        label="Film base spectral tilt",
        tier="basic",
        step=0.05,
        tooltip="Linear spectral-density multiplier tilt around the 555 nm pivot; 0.0 is neutral.",
    ),
)
...
DISPLAY_PANEL_FIELDS = (
    ParamSpec(
        ...
        tooltip="Use Pillow.ImageCms, with a macOS CoreGraphics fallback, to apply the display ICC profile to the napari viewer output; if disabled the output color space is used",
    ),
    ParamSpec(
        "gray_18_canvas",
