...
1. [src/spektrafilm](src/spektrafilm): runtime simulation pipeline (the
   physical-pipeline core), processed profile consumption, GPU/HDR helpers, and
   utility code. Linear-in / linear-out, no GUI or LUT-creator dependency.
...
3. `spektrafilm` (runtime) does not depend on any higher-level package.
...
I recommend installing spektrafilm with `conda`+`pip` or `uv` for now because
Python 3.13 is the tested interpreter line in this workspace.
